...
--source: 指定用于人脸检测的图像或视频的路径，或使用"camera"调用摄像头。默认值为"camera"。
...
注意：在运行示例3和4时，请确保将"path/to/image.jpg"或"path/to/video.mp4"替换为您实际要处理的图像或视频文件的路径。
"""
...
def main(model_choice, source):
    ...
    if source.lower() == 'camera':
        ...
        raise ValueError("Invalid source. Please specify an image path, video path, or 'camera'.")
    process_video(cap)
...
def process_image(img):
    ...
    if cv_detector is not None:  # Haar级联分类器
        # 检测人脸
        faces = haar_detect_faces(img, cv_detector)
    else:  # ResNet
        # 将图像从BGR转换为RGB
    ...
    face = get_face(img, faces)
    draw_faces_rectangles(img, faces)
    if face is not None:
        emoji = get_emoji(face)
        result_image = concatenate_images(img, face, emoji, 10)
        cv2.imshow('Face-Emoji', result_image)
        cv2.waitKey(0)

        result_dir = './result/'
        os.makedirs(result_dir, exist_ok=True)
        timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        result_image_path = os.path.join(result_dir, f'{timestamp}.jpg')
...
def process_video(cap):
    ...
    os.makedirs(result_dir, exist_ok=True)
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    result_video_path = os.path.join(result_dir, f'{timestamp}.avi')
    ...
    while True:
        ...
        if face is not None:
            emoji = get_emoji(face)
            result_frame = concatenate_images(frame, face, emoji, 10)
            cv2.imshow('Face-Emoji', result_frame)
            if not video_writer_created:
                height, width = result_frame.shape[:2]
                video_writer = cv2.VideoWriter(result_video_path, fourcc, 20.0, (width, height))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='camera',
                        help="Specify the source for face detection: image path, video path, or 'camera' (default: camera)")
